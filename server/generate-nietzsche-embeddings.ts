...
async function main() {
  console.log("⚡ Generating Friedrich Nietzsche embeddings...\n");
  
  // Delete existing Nietzsche embeddings from Common Fund
  console.log("🗑️  Clearing existing Nietzsche embeddings...");
  await db.delete(paperChunks).where(
    and(
      eq(paperChunks.figureId, 'common'),
      eq(paperChunks.author, 'Friedrich Nietzsche')
    )
  );
  console.log("✓ Cleared\n");
  ...
  try {
    console.log(`📄 Processing: Basic Writings of Nietzsche`);
    
    const content = readFileSync(
      join(__dirname, "../attached_assets/Basic Writings of Nietzsche (Modern Library Classics)_1762990311837.txt"), 
      "utf-8"
    );
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          await db.insert(paperChunks).values({
            ...
            paperTitle: "Basic Writings of Nietzsche",
            content: batch[j],
    ...
    console.log(`\n🎉 Done! Generated ${processedCount} Nietzsche embeddings in ${duration} minutes.`);
    console.log(`💾 Stored in Common Fund with author='Friedrich Nietzsche'`);

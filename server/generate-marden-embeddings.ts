...
async function main() {
  console.log("Generating Orison Swett Marden embeddings...\n");
  
  console.log("Clearing existing Marden embeddings...");
  await db.delete(paperChunks).where(
    and(
      eq(paperChunks.figureId, 'common'),
      eq(paperChunks.author, 'Orison Swett Marden')
    )
  );
  console.log("✓ Cleared\n");
  ...
  try {
    console.log(`Processing: Pushing to the Front`);
    
    const content = readFileSync(
      join(__dirname, "../attached_assets/Pasted-The-Project-Gutenberg-eBook-of-Pushing-to-the-Front-This-ebook-is-for-the-use-of-anyone-anywh-1762994087250_1762994087258.txt"),
      "utf-8"
    );
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          await db.insert(paperChunks).values({
            ...
            paperTitle: "Pushing to the Front",
            content: batch[j],
    ...
    console.log(`\nDone! Generated ${processedCount} Marden embeddings in ${duration} minutes.`);
    console.log(`Stored in Common Fund with author='Orison Swett Marden'`);

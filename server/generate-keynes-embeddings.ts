...
async function generateEmbeddingsBatch(texts: string[]): Promise<number[][]> {
  try {
    ...
    if (error?.status === 400 && errorMessage.includes('maximum context length')) {
      console.log(` WARNING: Batch too large`);
      throw error;
    }
    throw error;
  }
...
async function main() {
  console.log("Generating John Maynard Keynes embeddings...\n");
  
  console.log("Clearing existing Keynes embeddings...");
  ...
  try {
    console.log(`Processing: The Economic Consequences of the Peace`);
    
    const content = readFileSync(
      join(__dirname, "../attached_assets/Pasted-KEYNES-The-Project-Gutenberg-eBook-of-The-Economic-Consequences-of-the-Peace-This-ebook-is-1762992895604_1762992895610.txt"),
      "utf-8"
    );
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          await db.insert(paperChunks).values({
            ...
            paperTitle: "The Economic Consequences of the Peace",
            content: batch[j],
    ...
    console.log(`\nDone! Generated ${processedCount} Keynes embeddings in ${duration} minutes.`);
    console.log(`Stored in Common Fund with author='John Maynard Keynes'`);

...
async function main() {
  console.log("📚 Generating Bertrand Russell embeddings...\n");
  
  const dataDir = join(__dirname, "data/russell");
  const files = readdirSync(dataDir).filter(f => f.endsWith('.txt'));
  
  console.log(`📚 Found ${files.length} Russell file(s) to process\n`);
  ...
  for (const file of files) {
    ...
    const existing = await db.select().from(paperChunks)
      .where(and(eq(paperChunks.figureId, "russell"), eq(paperChunks.paperTitle, paperTitle)))
      .limit(1);

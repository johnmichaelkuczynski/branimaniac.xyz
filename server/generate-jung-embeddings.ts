...
async function main() {
  console.log("🧠 Generating Carl Jung embeddings...\n");
  
  // Delete existing Jung embeddings only
  console.log("🗑️  Clearing existing Jung embeddings...");
  await db.delete(paperChunks).where(eq(paperChunks.figureId, 'jung'));
  console.log("✓ Cleared\n");
  
  const startTime = Date.now();
  
  try {
    console.log(`📄 Processing: Psychology of the Unconscious`);
    
    const content = readFileSync(join(__dirname, "jung_psychology_unconscious.txt"), "utf-8");
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await db.insert(paperChunks).values({
        figureId: "jung",
        paperTitle: "Psychology of the Unconscious",
        content: chunk,
    ...
    console.log(`\n🎉 Done! Generated ${chunks.length} Jung embeddings in ${duration} minutes.`);
  } catch (error) {

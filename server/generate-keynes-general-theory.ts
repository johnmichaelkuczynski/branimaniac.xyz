...
async function main() {
  console.log("Adding Keynes General Theory embeddings...\n");
  
  // Get current highest chunkIndex for Keynes
  const existingKeynes = await db.query.paperChunks.findMany({
    where: and(
      eq(paperChunks.figureId, 'common'),
      eq(paperChunks.author, 'John Maynard Keynes')
    ),
    orderBy: (chunks, { desc }) => [desc(chunks.chunkIndex)],
    limit: 1
  });
  
  const startIndex = existingKeynes.length > 0 ? (existingKeynes[0].chunkIndex || 0) + 1 : 0;
  console.log(`Starting from chunk index: ${startIndex}\n`);
  
  const startTime = Date.now();
  
  try {
    console.log(`Processing: The General Theory of Employment, Interest and Money`);
    
    const content = readFileSync(
      join(__dirname, "../attached_assets/The General Theory of Employment, Interest and Money_1762993102798.txt"),
      "utf-8"
    );
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          await db.insert(paperChunks).values({
            ...
            paperTitle: "The General Theory of Employment, Interest and Money",
            content: batch[j],
    ...
    console.log(`\nDone! Generated ${processedCount} additional Keynes embeddings in ${duration} minutes.`);
    console.log(`Total Keynes chunks now: ${startIndex + processedCount}`);

...
async function main() {
  console.log("Generating John Locke embeddings...\n");
  
  console.log("Clearing existing Locke embeddings...");
  await db.delete(paperChunks).where(
    and(
      eq(paperChunks.figureId, 'common'),
      eq(paperChunks.author, 'John Locke')
    )
  );
  console.log("✓ Cleared\n");
  ...
  try {
    console.log(`Processing: Second Treatise of Government`);
    
    const content = readFileSync(
      join(__dirname, "../attached_assets/Pasted-LOCKE-The-Project-Gutenberg-eBook-of-Second-Treatise-of-Government-This-ebook-is-for-the-u-1762993304179_1762993304187.txt"),
      "utf-8"
    );
      ...
      try {
        ...
        for (let j = 0; j < batch.length; j++) {
          await db.insert(paperChunks).values({
            figureId: "common",
            author: "John Locke",
            paperTitle: "Second Treatise of Government",
            content: batch[j],
    ...
    console.log(`\nDone! Generated ${processedCount} Locke embeddings in ${duration} minutes.`);
    console.log(`Stored in Common Fund with author='John Locke'`);

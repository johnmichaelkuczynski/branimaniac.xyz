...
async function main() {
  console.log("🔬 Generating Henri Bergson embeddings...\n");
  
  // Delete existing Bergson embeddings only
  console.log("🗑️  Clearing existing Bergson embeddings...");
  await db.delete(paperChunks).where(eq(paperChunks.figureId, 'bergson'));
  console.log("✓ Cleared\n");
  
  const startTime = Date.now();
  
  try {
    console.log(`📄 Processing: The Henri Bergson Megapack (5 Classic Works)`);
    
    const content = readFileSync(join(__dirname, "bergson_works.txt"), "utf-8");
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await db.insert(paperChunks).values({
        figureId: "bergson",
        paperTitle: "The Henri Bergson Megapack",
        content: chunk,
    ...
    console.log(`\n🎉 Done! Generated ${chunks.length} Bergson embeddings in ${duration} minutes.`);
  } catch (error) {

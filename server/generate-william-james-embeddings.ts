...
async function main() {
  console.log("🧠 Generating William James embeddings...\n");
  
  // Delete existing William James embeddings only
  console.log("🗑️  Clearing existing William James embeddings...");
  await db.delete(paperChunks).where(eq(paperChunks.figureId, 'william-james'));
  console.log("✓ Cleared\n");
  
  const startTime = Date.now();
  
  try {
    console.log(`📄 Processing: William James Complete Works (Principles of Psychology & Will to Believe)`);
    
    const content = readFileSync(join(__dirname, "william_james_complete_works.txt"), "utf-8");
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      await db.insert(paperChunks).values({
        ...
        paperTitle: "The Principles of Psychology & The Will to Believe and Other Essays",
        content: chunk,
    ...
    console.log(`\n🎉 Done! Generated ${chunks.length} William James embeddings in ${duration} minutes.`);
  } catch (error) {

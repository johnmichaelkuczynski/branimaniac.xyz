...
async function main() {
  console.log("Generating Niccolò Machiavelli embeddings...\n");
  
  console.log("Clearing existing Machiavelli embeddings...");
  await db.delete(paperChunks).where(
    and(
      eq(paperChunks.figureId, 'common'),
      eq(paperChunks.author, 'Niccolò Machiavelli')
    )
  );
  console.log("✓ Cleared\n");
  ...
  try {
    console.log(`Processing: The Prince`);
    
    const content = readFileSync(
      join(__dirname, "../attached_assets/Pasted-MACHIAVELLI-The-Project-Gutenberg-eBook-of-The-Prince-This-ebook-is-for-the-use-of-anyone--1762993518014_1762993518023.txt"),
      "utf-8"
    );
    ...
    console.log(`\nDone! Generated ${processedCount} Machiavelli embeddings in ${duration} minutes.`);
    console.log(`Stored in Common Fund with author='Niccolò Machiavelli'`);

...
async function main() {
  console.log("👥 Generating Gustave Le Bon embeddings...\n");
  
  // Delete existing Le Bon embeddings from Common Fund
  console.log("🗑️  Clearing existing Le Bon embeddings...");
  await db.delete(paperChunks).where(
    and(
      eq(paperChunks.figureId, 'common'),
      eq(paperChunks.author, 'Gustave Le Bon')
    )
  );
  console.log("✓ Cleared\n");
  ...
  try {
    ...
    totalChunks += await processBook(
      join(__dirname, "../attached_assets/The Psychology of Revolution_1762990335195.txt"),
      "The Psychology of Revolution",
      totalChunks
    );
    
    totalChunks += await processBook(
      join(__dirname, "../attached_assets/The Crowd; study of the popular mind_1762990595468.txt"),
      "The Crowd: A Study of the Popular Mind",
      totalChunks
    );
    
    totalChunks += await processBook(
      join(__dirname, "../attached_assets/The evolution of forces_1762990595468.txt"),
      "The Evolution of Forces",
      totalChunks
    );
    
    const duration = ((Date.now() - startTime) / 1000 / 60).toFixed(1);
    console.log(`\n🎉 Done! Generated ${totalChunks} Le Bon embeddings in ${duration} minutes.`);
    console.log(`💾 Stored in Common Fund with author='Gustave Le Bon'`);

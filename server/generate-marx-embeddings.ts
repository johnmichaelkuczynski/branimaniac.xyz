...
async function main() {
  console.log("Generating Karl Marx embeddings...\n");
  
  console.log("Clearing existing Marx embeddings...");
  await db.delete(paperChunks).where(
    and(
      eq(paperChunks.figureId, 'common'),
      eq(paperChunks.author, 'Karl Marx')
    )
  );
  console.log("✓ Cleared\n");
  ...
  try {
    totalChunks += await processBook(
      join(__dirname, "../attached_assets/Pasted-MARX-The-Project-Gutenberg-eBook-of-The-Communist-Manifesto-This-ebook-is-for-the-use-of-a-1762992604137_1762992604144.txt"),
      "The Communist Manifesto",
      totalChunks
    );
    
    totalChunks += await processBook(
      join(__dirname, "../attached_assets/THE CAPITAL - Karl Marx_1762992699349.txt"),
      "Das Kapital (Capital)",
      totalChunks
    );
    
    const duration = ((Date.now() - startTime) / 1000 / 60).toFixed(1);
    console.log(`\nDone! Generated ${totalChunks} Marx embeddings in ${duration} minutes.`);
    console.log(`Stored in Common Fund with author='Karl Marx'`);

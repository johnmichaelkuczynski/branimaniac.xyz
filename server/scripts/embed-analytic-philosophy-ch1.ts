...
async function embedAnalyticPhilosophyCh1() {
  console.log("Starting Analytic Philosophy Chapter 1 embedding...");
  
  const filePath = path.join(__dirname, "../data/kuczynski/Analytic_Philosophy_Ch1_Positions.txt");
  ...
  for (let i = 0; i < chunks.length; i++) {
    ...
    try {
      ...
      await db.insert(paperChunks).values({
        figureId: "common",
        author: "Kuczynski",
        paperTitle: "Analytic Philosophy Chapter 1",
        content: chunk.content,
        embedding: embedding,
        chunkIndex: i,
        domain: "philosophy",
        significance: "HIGH",
        sourceWork: `Analytic Philosophy Ch1 - ${chunk.section}`
      });
  ...
  console.log(`\n=== ANALYTIC PHILOSOPHY CH1 EMBEDDING COMPLETE ===`);
